...
def stop_product(machine_id: int, line_number: int, background_tasks: BackgroundTasks, db: Session = Depends(deps.get_db)):
    result = machine_product_history_service.stop_machine_production(db, machine_id, line_number)
    if not result:
        return {"message": "Line này hiện không chạy sản phẩm nào."}
    background_tasks.add_task(ws_manager.broadcast, f"REFRESH_MACHINE_{machine_id}_PRODUCT")
    background_tasks.add_task(ws_manager.broadcast, "REFRESH_MACHINES")
    return {"message": "Đã dừng sản xuất sản phẩm hiện tại."}

@router.get("/{machine_id}/current", response_model=List[MachineProductHistoryResponse])
def get_current_product(machine_id: int, db: Session = Depends(deps.get_db)):
    return machine_product_history_service.get_current_product_of_machine(db, machine_id)
...
# ==========================================
# [CẬP NHẬT] API XUẤT EXCEL - BỔ SUNG CỘT LINE
# ==========================================
...
def export_global_history(
    ...
    for r in records:
        export_data.append({
            ...
            "Line": r.line_number, # [BỔ SUNG] Cột Line
